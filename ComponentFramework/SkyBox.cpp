...
bool SkyBox::OnCreate() {
	...
	SDL_Surface *textureface;

	textureface = IMG_Load("posx.jpg");

	if (textureface == nullptr)
	{
		return false;
	}
	int mode = (textureface->format->BytesPerPixel == 4) ? GL_RGBA : GL_RGB;
	glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X,
		0, GL_RGB, textureface->w, textureface->h, 0, mode, GL_UNSIGNED_BYTE, textureface->pixels);
	SDL_FreeSurface(textureface);


	textureface = IMG_Load("negx.jpg");

	if (textureface == nullptr)
	{
		return false;
	}
	mode = (textureface->format->BytesPerPixel == 4) ? GL_RGBA : GL_RGB;
...
void SkyBox::Render(Matrix4 modelMatrix) const
{
